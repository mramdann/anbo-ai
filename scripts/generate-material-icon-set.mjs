...
const assetDir = resolve(projectRoot, "public/material-icons");
mkdirSync(assetDir, { recursive: true });
const urls = Object.fromEntries(Object.entries(icons).map(([name, icon]) => {
  if (!/^[a-z0-9-]+$/.test(name)) throw new Error(`Invalid icon name: ${name}`);
  const folder = name.startsWith("folder-") || name === "folder";
  const width = icon.width ?? (folder ? 16 : iconify.width ?? 24);
  const height = icon.height ?? (folder ? 16 : iconify.height ?? 24);
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${icon.left ?? 0} ${icon.top ?? 0} ${width} ${height}">${icon.body}</svg>`;
  writeFileSync(resolve(assetDir, `${name}.svg`), svg);
  const revision = createHash("sha256").update(svg).digest("hex").slice(0, 12);
  return [name, `/material-icons/${name}.svg?v=${revision}`];
}));

const output = `/* Generated by scripts/generate-material-icon-set.mjs. */
export const materialIconUrls: Record<string, string> = ${JSON.stringify(urls, null, 2)};
