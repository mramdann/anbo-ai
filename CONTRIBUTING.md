# Contributing to Anbo

Anbo is a solo-maintained project with a strong product direction. Contributions are welcome, but **alignment matters more than volume**.
...
- Anbo has one active maintainer ([@mramdann](https://github.com/mramdann)).
...
For the architecture and how to contribute safely, see [ANBO.md](ANBO.md) and the [docs/ index](docs/README.md).

## Where to discuss

Use [GitHub Issues](https://github.com/mramdann/anbo-ai/issues) for design discussion, scope questions, concrete bugs, and feature proposals.
...
Anbo positions itself as **lightweight, fast, production-grade**. Every PR is reviewed against:
...
The most common way a PR breaks Anbo is a **local fix with global blast radius**: the diff solves one reported case, reads fine, passes type-check and clippy, and silently breaks the same subsystem in every other case. Review alone does not catch these. A test does.
...
- Anbo is not trying to be a full IDE replacement (VS Code, Cursor, Zed).
