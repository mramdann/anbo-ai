...
# zsh, so we shadow $? into `_anbo_ret`.

{
  _anbo_user_zdotdir="${ANBO_USER_ZDOTDIR:-$HOME}"
  [ -f "$_anbo_user_zdotdir/.zshrc" ] && source "$_anbo_user_zdotdir/.zshrc"
...
if [[ -z "$__ANBO_HOOKS_LOADED" ]]; then
  __ANBO_HOOKS_LOADED=1
  ...
  _anbo_precmd() {
    local _anbo_ret=$?
    printf '\e]133;D;%s\e\\' "$_anbo_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_anbo_urlencode "$PWD")"
  ...
  _anbo_preexec() {
    ...
    [[ -n "$ANBO_BLOCKS" ]] && _anbo_block_seen=1
    local cmd="${1//[[:cntrl:]]/ }"
    printf '\e]133;C;%s\e\\' "${cmd[1,256]}"
  }

  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _anbo_precmd
    add-zsh-hook preexec _anbo_preexec
  fi
