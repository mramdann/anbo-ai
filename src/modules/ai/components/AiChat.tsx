    ...
    <Conversation>
      <ConversationContent className="gap-5 p-3">
        {messageWindow.hiddenCount > 0 ? (
          ...
          >
            Show{" "}
            {Math.min(DEFAULT_VISIBLE_MESSAGE_COUNT, messageWindow.hiddenCount)}{" "}
            previous messages
          </button>
        ) : null}
...
const RenderedMessage = memo(function RenderedMessage({
  ...
  if (message.role === "user") {
    ...
    const cmdMatch = rawText.match(ANBO_CMD_RE);
...
const ReadGroup = memo(function ReadGroup({ parts }: { parts: AnyPart[] }) {
  ...
  return (
    <Collapsible className="group/read overflow-hidden rounded-md border border-border/50 bg-card/50">
      ...
      <CollapsibleContent className="anbo-collapsible-content border-t border-border/30">
