...
export function AiComposerProvider({ children }: ProviderProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("anbo:ai-attach-file", onAttach);
    return () => window.removeEventListener("anbo:ai-attach-file", onAttach);
  ...
  const submit = () => {
    ...
    // the prompt to the ANBO.md scan template before sending.
