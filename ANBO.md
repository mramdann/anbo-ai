...
- **markdown/** - markdown preview renderer (backs the `markdown` tab kind). Preview IO and rendering are deferred until the tab is visible, and the backend enforces a 2 MiB preview limit before content crosses IPC; larger files remain available through raw editor mode. Streamdown stays in static mode with its sanitize/harden pipeline, explicit safe URL transformation, and link confirmation. Mermaid rendering is not enabled. The transitive Mermaid and DOMPurify versions are pinned through `pnpm-workspace.yaml` to patched releases, and production dependency advisories at moderate severity or above block both CI and release builds.
...
- **Partial, startup budget:** CI measures the complete local module entry, modulepreload, and stylesheet closure from generated `index.html` and `settings.html`, rejects missing or escaping assets, reports the largest contributors, and retains the total-client-JavaScript gate. Provider SDKs remain available through their existing dynamic imports but are forbidden from both startup closures. Correct Rolldown dependency grouping reduced the measured main closure from 789.19 KiB to 658.02 KiB gzip and Settings from 471.44 KiB to 292.47 KiB, protected by 670 KiB and 305 KiB ceilings. Continue with separately reviewed feature surfaces such as updater UI or LSP only when the flow and first-use behavior remain intact.
...
- **Partial, AI UI and context:** context compaction and bounded AI file reads exist, but completed and streaming conversations still render the full message list. Avoid scanning the full conversation on every streamed token, virtualize or collapse old messages, and enforce hard per-turn, per-session, and final model-context byte/token limits including system prompts and output reserve.
- **Partial, large content:** editor reads, syntax/LSP activation, Markdown preview IO, and AI file tools have conservative limits. Add equivalent limits or explicit opt-in for large fenced-code highlighting and AI diff views, using bounded plain-text or patch summaries by default.
- **Partial, async work:** some source-control and loading paths already use cancellation flags or request IDs. Apply context generations consistently to Explorer search, directory listing, breadcrumbs, session loading, key refresh, and every refresh that can outlive its source workspace. Coalesce continuous settings slider writes and verify bounded cache eviction.

### P3: delivery and tests

- **Open, supported updater platforms:** release automation is Windows-only. Do not prompt Linux/macOS users for packages that are absent. Either restore tested platform builds or explicitly scope updater behavior to supported artifacts.
...
- **Partial, integration tests:** SSRF redirect handling, symlinked project-memory rejection, frozen AI run context, LSP blocked-stdin shutdown, concurrent editor saves, backend HTTP cancellation, and cross-space tab isolation have regression coverage. Add direct privileged IPC rejection, explicit descendant-process assertions, browser lock/cancellation integration, and shutdown-cleanup integration tests.
- **Open, coverage thresholds:** frontend coverage is not enforced and Rust LCOV generation has no threshold. Prefer changed-line or subsystem thresholds for security and lifecycle invariants over a broad vanity percentage.
