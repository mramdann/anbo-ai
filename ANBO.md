...
- **browser/** - built-in browser tab (status-bar pill suggests opening when a localhost URL is detected).
  On Windows, overlapping React menus move the native browser child below a transparent main-webview surface with `browser_embed_set_ui_overlay`; do not hide, park, or repeatedly capture the child for menus because that freezes or throttles media playback. Dragging may still use a single freeze frame.
