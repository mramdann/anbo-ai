...
- **browser/** - built-in browser tab (status-bar pill suggests opening when a localhost URL is detected). Each warm browser tab keeps a stable renderer owner across React/Dockview host handoffs. `BrowserStack` mounts browser tabs that have no visible Dockview panel in a shared background host, including tabs in inactive spaces, so agents, navigation, and media continue without activating the workspace or stealing focus. Closing a UI tab calls `browser_embed_close` and destroys its native child; a temporary React unmount calls owner-scoped `browser_embed_release` instead.
  On Windows, inactive native browser children remain rendered at `HWND_BOTTOM` while the active child is placed at `HWND_TOP`. Do not replace this with `hide()` because WebView2 throttles hidden pages and can stall automation or media. Overlapping React menus temporarily move the active child below the transparent main-webview surface with `browser_embed_set_ui_overlay`; dragging may still use a single freeze frame. Browser tabs show a theme-aware pulsing automation indicator while a CLI or the AI panel is operating on them.
...
- **Open, native browser:** replace the permanent 25 Hz geometry/layout polling and broad DOM `MutationObserver` scans with resize, layout, visibility, and explicit overlay events plus a low-frequency fallback. Capture drag snapshots only for the active visible tab and release base64 freeze frames after drag.
