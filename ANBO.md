...
- `pnpm size` checks both total client JavaScript and the real startup transfer closure emitted into `dist/index.html` and `dist/settings.html`. `scripts/startup-budget.mjs` counts every local module entry, modulepreload, and stylesheet once with per-file gzip sizes. Current regression ceilings are 670 KiB for the main window and 305 KiB for Settings, and AI provider modules are forbidden in both startup closures. AI SDK and Dockview remain in their natural lazy graph. Editor and LSP modules, CodeMirror core, UIW themes, and Lezer facets share one recursively captured `editor-runtime` group so facet identity and base styles remain stable, while higher-priority shared runtime groups stay separate, language implementations stay lazy, and LSP processes remain opt-in. CI runs `pnpm smoke:production` after the Windows-target frontend build, rejects every static cycle between emitted JavaScript chunks, and requires the real entry bundle to finish evaluating in headless Chromium. Do not skip either gate or raise the limits to accommodate an unrelated change.
