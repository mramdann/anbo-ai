...
- **Open, startup budget:** measure the complete modulepreload and stylesheet closure from generated `index.html` and `settings.html`. The eager-graph check provides a dependency safeguard, but `.size-limit.json` still counts only selected chunk names and can miss eagerly preloaded provider, LSP, workspace, and store chunks. Lazy-load provider SDKs and feature surfaces not needed at startup.
