...
- **agents/** - agent launching, notifications, and management for both the built-in Anbo agent and terminal coding agents (Claude Code, Codex, Antigravity CLI, Pi, OpenCode, Grok). The header launcher (`components/AgentLauncherPanel.tsx` + `lib/launcher.ts`) persists per-agent start commands in preferences and atomically builds balanced one-to-four-pane tabs. Shared store (`store/agentStore.ts`: terminal `sessions` + `localAgent` + `notifications`) and a shared router (`lib/route.ts`: suppress when focused-and-visible, OS-notify when unfocused, in-app Sonner toast when focused-but-hidden) feed the header `NotificationBell` (management surface, Anbo agent listed first, per-agent hook enable rows). Toasts use Sonner (`components/ui/sonner.tsx`) themed via the central engine; `lib/agentIcon.tsx` renders the per-agent brand mark, including the official OpenAI Blossom for Codex, theme-aware official OpenCode square mark, current Pi favicon, theme-aware official SpaceXAI mark for Grok Build, and bundled official full-color Antigravity icon. Terminal detection is Rust-side (`pty/agent_detect.rs`) on the PTY reader's byte filter, armed on `OSC 133;C;<cmd>` or self-armed by the marker, emitting `anbo:agent-signal` transitions (`started`/`working`/`attention`/`finished`/`exited`) driven only by OSC sequences (never raw output, so a repainting TUI never flaps) - zero cost when no agent runs. `agy` maps to the internal `antigravity` identity. Hook-backed terminal agents converge on the same `OSC 777` marker the detector reads, installed via `agent_enable_hooks(agent)` / `agent_hooks_status(agent)` in `modules/agent.rs` (data-driven `AgentSpec` for JSON-hook agents plus a Anbo-owned Pi extension; atomic writes, foreign configuration preserved, idempotent; gated on `ANBO_TERMINAL`). OpenCode and Grok use OSC 133 process-lifecycle detection but do not install attention hooks. Delivery differs because only Claude's hook protocol can return terminal bytes in the hook *response*: **Claude** (`~/.claude/settings.json`, `UserPromptSubmit`/`Notification`/`Stop`) returns the marker via the `terminalSequence` field (legacy 3-field `notify;Anbo;<event>`). **Codex** (`~/.codex/hooks.json`, `UserPromptSubmit`/`PermissionRequest`/`Stop`) and **Antigravity** (`~/.gemini/config/hooks.json`, named `PreInvocation`/`Stop` hook definition) cannot, so the hook command emits the 4-field `notify;Anbo;<agent>;<event>` marker itself (`/dev/tty` on Unix or `CONOUT$` after `AttachConsole` on Windows) and prints `{}` as a JSON stdout no-op. Antigravity captures the camel-case `conversationId` and only reports `Stop` as finished when `fullyIdle` is not false. **Pi** (`~/.pi/agent/extensions/anbo-notifications.ts`) uses `agent_start`/`agent_settled` extension events and writes its named marker directly to stdout. The agent-named marker lets a self-arm name the right agent when no preexec fired (bash/tmux/Windows). The Anbo agent path is `ai/components/LocalAgentNotificationsBridge.tsx`, mapping `chatStore.agentMeta` (`awaiting-approval`→attention, busy→idle→finished, `error`) into the same router.
...
- OpenCode exact resume uses the Anbo-owned global plugin `~/.config/opencode/plugins/anbo-notifications.js`. Its `session.created` hook emits the session ID through OSC 777; the PTY detector routes that ID to the originating leaf before workspace serialization.
- Agent Launcher always sends the configured start command unchanged. It never appends a generated session flag. Claude obtains its real session ID from hook stdin, Antigravity obtains its conversation ID from hook stdin, Pi obtains its ID from the extension session manager, and OpenCode obtains its ID from `session.created`; only then does Anbo persist exact-session resume metadata. Restored Antigravity tabs use `agy --conversation <id>` rather than workspace-global `agy -c`, so concurrent tabs cannot resume one another's latest thread.
