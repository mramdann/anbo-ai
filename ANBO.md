...
- `pnpm size` checks both total client JavaScript and the real startup transfer closure emitted into `dist/index.html` and `dist/settings.html`. `scripts/startup-budget.mjs` counts every local module entry, modulepreload, and stylesheet once with per-file gzip sizes. Current regression ceilings are 670 KiB for the main window and 305 KiB for Settings, and every AI provider or shared AI SDK chunk is forbidden in both startup closures. Rolldown groups use `includeDependenciesRecursively: false`; `scripts/bundle-groups.mjs` classifies the package after the final `node_modules` segment so a pnpm parent path cannot pull React or other shared runtime dependencies into a lazy provider chunk. Dockview and dockview-react must remain outside manual chunk groups: forcing them into a named chunk creates a production-only circular evaluation through shared application chunks. CI runs `pnpm smoke:production` after the Windows-target frontend build and requires the real entry bundle to finish evaluating in headless Chromium. Do not replace this with selected chunk-name globs, skip the runtime smoke, or raise the limits to accommodate an unrelated change.
