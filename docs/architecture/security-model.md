...
Anbo runs shells, reads and writes files, and sends data to AI providers. The security model is defense-in-depth: no single guard is enough, so every boundary validates input before acting on it.
...
Local LLM endpoints are explicitly allowed because the user opted in by pointing Anbo at them, but they are still classified and logged.
...
Service constant: `anbo-ai`. Keys never touch disk outside the keychain/Linux secrets file, never go in `localStorage`, and never appear in logs.
