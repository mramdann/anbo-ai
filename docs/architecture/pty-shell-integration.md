...
This guide elaborates on `ANBO.md`. If anything here conflicts with `ANBO.md`, `ANBO.md` wins.
...
- `init.fish` for fish, copied to Anbo's private runtime directory and loaded with fish `-C`

Zsh is launched with `ZDOTDIR` pointing at a temp directory that sources our scripts and then the user's real configs. Bash uses `--rcfile` with a wrapper that sources the user's `~/.bashrc` after Anbo's. Fish loads the private runtime script with `-C`, so no user file is replaced.

All integrated shells emit **OSC 7** (cwd) and **OSC 133 A/B/C/D** (prompt boundaries and exit code) so Anbo can track cwd and detect command boundaries without parsing the user's prompt.
...
Fish 4.0 writes its own OSC 133 prompt markers. To avoid doubling, Anbo sets `fish_features=no-mark-prompt` and re-asserts its own prompt via `-C` after `config.fish` runs.
